...
BEGIN
	...
	SET @basic_query = CONCAT("select *, if(@v2 = indicator_id or @v2 = 0, @v1:= @v1 + 1, @v1:= 1) AS executions_order, @v2:= indicator_id AS v2   from dashboard_indicatordata ", filter , " order by indicator_id, load_date DESC");
